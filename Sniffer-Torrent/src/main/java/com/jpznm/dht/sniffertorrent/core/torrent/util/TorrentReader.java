package com.jpznm.dht.sniffertorrent.core.torrent.util;
...
public class TorrentReader {
    ...
    @Getter
    private byte[] torrentBuffer = null;
     ...
     * @throws IOException When the info dictionary can't be read or encoded and hashed back
     *                     to create the torrent's SHA-1 hash.
     */
    public TorrentReader(byte[] torrent, boolean seeder) throws IOException, NoSuchAlgorithmException {

        this.torrentBuffer = torrent;
        ...
        this.info_hash = TorrentReader.hash(this.encoded_info);
        this.hex_info_hash = BytesUtil.binToHex(this.info_hash);

        /**
         * Parses the announce information from the decoded meta-info structure.
         *
         * <p>
         * If the torrent doesn't define an announce-list, use the mandatory announce
         * field value as the single tracker in a single announce tier. Otherwise, the
         * announce-list must be parsed and the trackers from each tier extracted.
         * </p>
         *
         * @see <a href="http://bittorrent.org/beps/bep_0012.html">BitTorrent BEP#0012
         *      "Multitracker Metadata Extension"</a>
         */
        try {
        ...
        if (this.decoded_info.containsKey("files")) {
            for (BEValue file : this.decoded_info.get("files").getList()) {
                ...
                this.files.add(new TorrentFile(new File(this.name, path.toString()), fileInfo.get("length").getLong()));
            }
        } else {
    ...
    public static String toHexString(String input) {
        try {
            byte[] bytes = input.getBytes(TorrentReader.BYTE_ENCODING);
            return BytesUtil.binToHex(bytes);
     ...
     * value greater than 0, its value will be used. Otherwise, it defaults to the
     * number of processors detected by the Java Runtime.
     * </p>
     *
     ...
     */
    public static TorrentReader load(File torrent) throws IOException, NoSuchAlgorithmException {
        return TorrentReader.load(torrent, false);
    }
     ...
     * @param torrent The abstract {@link File} object representing the
     *                <tt>.torrent</tt> file to load.
     * @param seeder  Whether we are a seeder for this torrent or not (disables local
     *                data validation).
     * @throws IOException When the torrent file cannot be read.
     */
    public static TorrentReader load(File torrent, boolean seeder) throws IOException, NoSuchAlgorithmException {
        byte[] data = FileUtils.readFileToByteArray(torrent);
        return new TorrentReader(data, seeder);
    }

    /** Torrent creation --------------------------------------------------- */

    /**
     * Create a {@link TorrentReader} object for a file.
     *
     * <p>
     * Hash the given file to create the {@link TorrentReader} object representing
     * the Torrent metainfo about this file, needed for announcing and/or sharing
     * said file.
     * </p>
     *
     * @param source    The file to use in the torrent.
     * @param announce  The announce URI that will be used for this torrent.
     * @param createdBy The creator's name, or any string identifying the torrent's
     *                  creator.
     */
    public static TorrentReader create(File source, URI announce, String createdBy)
            throws InterruptedException, IOException, NoSuchAlgorithmException {
        return TorrentReader.create(source, null, DEFAULT_PIECE_LENGTH, announce, null, createdBy);
    }

    /**
     * Create a {@link TorrentReader} object for a set of files.
     *
     * <p>
     * Hash the given files to create the multi-file {@link TorrentReader} object
     * representing the Torrent meta-info about them, needed for announcing and/or
     * sharing these files. Since we created the torrent, we're considering we'll be
     * a full initial seeder for it.
     * </p>
     *
     * @param parent    The parent directory or location of the torrent files, also used
     ...
     *                  creator.
     */
    public static TorrentReader create(File parent, List<File> files, URI announce, String createdBy)
            throws InterruptedException, IOException, NoSuchAlgorithmException {
        return TorrentReader.create(parent, files, DEFAULT_PIECE_LENGTH, announce, null, createdBy);
    }
     ...
     *
     * @param source       The file to use in the torrent.
     * @param announceList The announce URIs organized as tiers that will be used for this
     *                     torrent
     * @param createdBy    The creator's name, or any string identifying the torrent's
     *                     creator.
     */
    public static TorrentReader create(File source, int pieceLength, List<List<URI>> announceList, String createdBy)
            throws InterruptedException, IOException, NoSuchAlgorithmException {
        return TorrentReader.create(source, null, pieceLength, null, announceList, createdBy);
    }
     ...
     * </p>
     *
     * @param source       The parent directory or location of the torrent files, also used
     *                     as the torrent's name.
     * @param files        The files to add into this torrent.
     * @param announceList The announce URIs organized as tiers that will be used for this
     *                     torrent
     ...
     *                     creator.
     */
    public static TorrentReader create(File source, List<File> files, int pieceLength, List<List<URI>> announceList,
                                       String createdBy) throws InterruptedException, IOException, NoSuchAlgorithmException {
        return TorrentReader.create(source, files, pieceLength, null, announceList, createdBy);
    }

    /**
     * Helper method to create a {@link TorrentReader} object for a set of files.
     *
     * <p>
     * Hash the given files to create the multi-file {@link TorrentReader} object
     * representing the Torrent meta-info about them, needed for announcing and/or
     ...
     *                     creator.
     */
    private static TorrentReader create(File parent, List<File> files, int pieceLength, URI announce,
        ...
        if (files == null || files.isEmpty()) {
            ...
            info.put("pieces", new BEValue(TorrentReader.hashFile(parent, pieceLength), TorrentReader.BYTE_ENCODING));
        } else {
            ...
            info.put("pieces", new BEValue(TorrentReader.hashFiles(files, pieceLength), TorrentReader.BYTE_ENCODING));
        }
        torrent.put("info", new BEValue(info));

        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        BEncoder.bencode(new BEValue(torrent), baos);
        return new TorrentReader(baos.toByteArray(), true);
    }
    ...
    private static class CallableChunkHasher implements Callable<String> {
        ...
        public String call() throws UnsupportedEncodingException {
            ...
            return new String(md.digest(), TorrentReader.BYTE_ENCODING);
        }
    }
     ...
     *
     * <p>
     * Hashes the given file piece by piece using the default Torrent piece length
     * (see {@link #PIECE_LENGTH}) and returns the concatenation of these hashes, as
     * a string.
     * </p>
     *
     * <p>
    ...
    private static String hashFile(File file, int pieceLenght)
            throws InterruptedException, IOException, NoSuchAlgorithmException {
        return TorrentReader.hashFiles(Arrays.asList(new File[]{file}), pieceLenght);
    }
