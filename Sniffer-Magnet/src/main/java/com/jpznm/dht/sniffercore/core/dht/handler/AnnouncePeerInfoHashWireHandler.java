...
import com.jpznm.dht.sniffercore.core.dht.util.ByteUtil;
import com.jpznm.dht.sniffercore.core.dht.util.ExtensionUtil;
import com.jpznm.dht.sniffercore.core.dht.util.StringUtil;
...
public class AnnouncePeerInfoHashWireHandler implements IInfoHashHandler {
    ...
    private final byte[] BT_RESERVED = new byte[]{(byte) (0x00 & 0xff), (byte) (0x00 & 0xff), (byte) (0x00 & 0xff),
            (byte) (0x00 & 0xff), (byte) (0x00 & 0xff), (byte) (0x10 & 0xff), (byte) (0x00 & 0xff),
            (byte) (0x01 & 0xff),};
    ...
    private void sendMessage(int id, byte[] data) throws IOException {
        ...
        // extended message ID. 0 = handshake, >0 = extended message as specified by the
        // handshake.
    ...
    private void onExtendHandShake(Map<String, Object> map) throws IOException {
        ...
        if (m == null || !m.containsKey("ut_metadata") || !map.containsKey("metadata_size")) {
            log.error("onExtendHandShake failed");
            release();
            return;
        }
    ...
    private void onPiece(byte[] buff) throws IOException {
        ...
        if (!map.containsKey("msg_type") || !map.containsKey("piece")) {
            log.error("onPiece packet error.");
            release();
            return;
        }

        if (((BigInteger) map.get("msg_type")).intValue() != 1) {
            log.error("onPiece error, msg_type:" + map.get("msg_type"));
            release();
            return;
        }
    ...
    private Torrent parseTorrent(Map<String, Object> map) throws UnsupportedEncodingException {
        ...
        if (info.containsKey("files")) {
            ...
            String temp = StringUtil.join(",", types.toArray(new String[]{}));
